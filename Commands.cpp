...
void Server::PASS(const Message &obj) {
    ...
    while (it != this->_conClients.end()) {
        if (this->_fd_client == it->first) {
            if (M_DEBUG) {
                std::cout << "COMMAND: *PASS* FUNCTION GOT TRIGGERT"
                          << std::endl;
                std::cout << std::endl;
            }
            Client obj = it->second;

            // the password belongs to the server so check if there is a
            // password set (!"") and then check if correct or not
...
void Server::PART(const Message &obj) {
    ...
    if (M_DEBUG) std::cout << "TRIGGERED PART" << std::endl;
    ...
    for (iterator end(_v_channels.end()); begin < end; begin++) {
        ...
        for (str_iterator param_end(tree[0].end()); param_begin < param_end;
             param_begin++) {
...
void Server::ChannelFlags(const Message &obj,
                          std::vector<std::vector<std::string> > tree,
                          bool sign) {
    (void)obj;
    std::vector<Channel>::iterator first_channel;
    for (std::vector<Channel>::iterator channel_end;
         first_channel < channel_end; first_channel++) {
        if ((*first_channel).getName() == tree[0][0].substr(1)) {
            std::string::iterator flags(tree[1][0].begin() + 1);
            for (std::string::iterator end(tree[1][0].end()); flags < end;
                 flags++)
...
void Server::MODE(const Message &obj) {
    ...
    {
        if (tree[0][0][0] != '&' && tree[0][0][0] != '#') is_channel = false;
    }

    bool sign = true;
    {
        if (tree[1][0][0] == '-') sign = false;
    }
    ...
    if (is_channel) {
        ...
        // std::vector<Channel>::iterator	first_channel;
        // for (std::vector<Channel>::iterator channel_end; first_channel <
        // channel_end; first_channel++)
        // {
        // 	if ( (*first_channel).getName() == tree[0][0].substr(1))
        // 	{
        // 		std::string::iterator	flags(tree[1][0].begin() + 1);
        // 		for (std::string::iterator	end(tree[1][0].end());
        // flags < end; flags++)
        // 			(*first_channel).setChannelRule(*flags, sign);
        // 	}
        // }
...
void Server::JOIN(const Message &obj) {
    ...
    for (viterator chanelname1(tree[0].begin()); chanelname1 < chanelname2;
         chanelname1++) {
        Channel *chany = NULL;
        {
            std::vector<Channel>::iterator chanel_list2(_v_channels.end());
            for (std::vector<Channel>::iterator chanel_list1(
                     _v_channels.begin());
                 chanel_list1 < chanel_list2; chanel_list1++) {
        ...
        if (chany) {
            ...
            // check if banned
            if (chany->isClientRight(_conClients[_fd_client].getUsername(),
                                     'b')) {
            ...
            _v_channels[_v_channels.size() - 1].add_client(
                _conClients[_fd_client]);
        }
        key++;
    }
    if (M_DEBUG) {
        std::cout << "The socketid of the caller: " << _fd_client << std::endl;
        std::cout << "Through the map: " << _conClients[_fd_client].getSocket()
                  << std::endl;
    }

    std::vector<Channel>::iterator end(_v_channels.end());
    for (std::vector<Channel>::iterator begin(_v_channels.begin()); begin < end;
         begin++)
...
// 	{
// 		this->sendMessage(cl, ERR_NEEDMOREPARAMS(cl,
// ERR_NOSUCHCHANNEL(cl, msg.getParameters()[0]))); 		return ;
// 	}
// 	Channel *ch = &this->_channels.at(msg.getParameters()[0]);
// 	if (!) // make member function of this
// 	{
// 		this->sendMessage(cl, ERR_NOTONCHANNEL(cl,
// msg.getParameters()[0])); 		return ;
// 	}
...
// 	{
// 		if (/*not operator*/)
// 			this->sendMessage(cl, ERR_CHANOPRIVSNEEDED(cl,
// ch->getName())); 		else
